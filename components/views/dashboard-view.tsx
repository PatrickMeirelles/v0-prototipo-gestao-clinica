...
import { TrendingUp, TrendingDown, Building2, FileText, Filter, PackageX } from "lucide-react"
import { 
  BarChart, 
  Bar, 
  XAxis, 
...
export function DashboardView() {
  ...
  const maiorFornecedor = useMemo(() => {
    ...
    filteredTransactions.forEach(t => {
      gastosPorFornecedor[t.fornecedor] = (gastosPorFornecedor[t.fornecedor] || 0) + t.valorTotal
    })
  ...
  const variacao = useMemo(() => {
    ...
    const gastoAnterior = transacoesAnterior.reduce((acc, t) => acc + t.valorTotal, 0)
  ...
  const categoryData = useMemo(() => {
    ...
    return [
      { name: "Medicações", value: Math.round((totalMedicacao / total) * 100), color: "#0d9488" },
      { name: "Fretes", value: Math.round((totalFrete / total) * 100), color: "#f97316" },
    ]
  }, [filteredTransactions])
  ...
  const monthlyData = useMemo(() => {
    ...
    const nomesAbreviados = ["Jan", "Fev", "Mar", "Abr", "Mai", "Jun", "Jul", "Ago", "Set", "Out", "Nov", "Dez"]
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const transacoesMes = mockTransactions.filter(t => t.mes === mesStr && t.ano === anoStr)
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        <Card className="border-slate-200 bg-white">
          ...
          <CardContent>
            ...
            {hasData && variacao !== 0 && (
              <p className={`text-xs ${variacao >= 0 ? "text-emerald-600" : "text-rose-600"}`}>
                {variacao >= 0 ? "+" : ""}{variacao}% em relação ao mês anterior
              </p>
            )}
